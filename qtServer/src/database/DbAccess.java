...
public class DbAccess {
     ...
     */
    public DbAccess() {
        this.SERVER = readConfig("QT_DB_HOST", "qt.db.host", SERVER);
        this.DATABASE = readConfig("QT_DB_NAME", "qt.db.name", DATABASE);
        this.PORT = readConfig("QT_DB_PORT", "qt.db.port", PORT);
        this.USER_ID = readConfig("QT_DB_USER", "qt.db.user", null);
        this.PASSWORD = readConfig("QT_DB_PASSWORD", "qt.db.password", null);
    }
        ...
        try {
            if (USER_ID == null || USER_ID.isBlank()) {
                throw new DatabaseConnectionException(
                        "Username database non configurato. Impostare QT_DB_USER o qt.db.user");
            }
            if (PASSWORD == null || PASSWORD.isBlank()) {
                throw new DatabaseConnectionException(
                        "Password database non configurata. Impostare QT_DB_PASSWORD o qt.db.password");
            }
    ...
    public static String requireSafeIdentifier(String identifier, String label) {
        if (identifier == null || identifier.isBlank()) {
            throw new IllegalArgumentException(label + " non puo essere vuoto");
        }
        if (!identifier.matches(IDENTIFIER_REGEX)) {
            throw new IllegalArgumentException(label + " non valido: " + identifier);
        }
        return identifier;
    }

    public static String quoteIdentifier(String identifier, String label) {
        return "`" + requireSafeIdentifier(identifier, label) + "`";
    }
