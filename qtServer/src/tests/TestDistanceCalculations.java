...
public class TestDistanceCalculations {
    ...
    private static void testDiscreteItemDistance() {
        ...
        try {
            ...
            assertEqual(item1.distance(item2.getValue()), 0.0,
                       "Distanza tra item uguali dovrebbe essere 0");

            // Distanza tra item diversi dovrebbe essere 1
            assertEqual(item1.distance(item3.getValue()), 1.0,
                       "Distanza tra item diversi dovrebbe essere 1");
    ...
    private static void testContinuousItemDistance() {
        ...
        try {
            ...
            assertTrue(distance >= 0.0 && distance <= 1.0,
                      "Distanza normalizzata dovrebbe essere in [0,1]");

            // Distance dovrebbe essere (30-20)/(40-0) = 10/40 = 0.25
            assertTrue(Math.abs(distance - 0.25) < 0.0001,
                      "Distanza calcolata non corretta");
    ...
    private static void testTupleDistance() {
        ...
        try {
            ...
            assertEqual(tuple1.getDistance(tuple2), 0.0,
                       "Distanza tupla con se stessa dovrebbe essere 0");
    ...
    private static void testDistanceSymmetry() {
        ...
        try {
            ...
            assertTrue(Math.abs(dist1to2 - dist2to1) < 0.0001,
                      "Distanza dovrebbe essere simmetrica");
            ...
            for (int i = 0; i < numExamples; i++) {
                for (int j = i + 1; j < numExamples; j++) {
                    ...
                    if (distance < 0.0 || distance > 1.0) {
                        allInRange = false;
                        System.out.println("  ✗ Distanza fuori range: tuple[" + i +
                                         "][" + j + "] = " + distance);
                    }
                }
            }
